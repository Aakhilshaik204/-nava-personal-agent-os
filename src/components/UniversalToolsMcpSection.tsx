...
interface McpTemplate {
  ...
  envKeys: string[];
  category: string;
  icon: 'github' | 'postgres' | 'slack' | 'db' | 'custom' | 'code' | 'browser' | 'terminal' | 'audit' | 'mail' | 'search' | 'doc';
...
const DEFAULT_MCP_TEMPLATES: McpTemplate[] = [
  {
    id: 'tpl-context7',
    name: 'Context7 Symbol Graph MCP',
    description: 'AST repository symbol graphs and context definition slicing for codebases.',
    command: 'npx',
    args: ['-y', '@context7/mcp-server@latest'],
    envKeys: [],
    category: 'Code & AST',
    icon: 'code',
    toolsDiscovered: ['context7.get_symbol_graph', 'context7.slice_context'],
  },
  {
    id: 'tpl-superpowers',
    name: 'Superpowers Structural Code MCP',
    description: 'Syntax-aware AST search, pattern replacement, and compiler auto-fix.',
    command: 'uvx',
    args: ['mcp-superpowers-code@latest'],
    envKeys: [],
    category: 'Code & AST',
    icon: 'code',
    toolsDiscovered: ['superpowers.ast_search', 'superpowers.ast_replace', 'superpowers.compiler_autofix'],
  },
  {
    id: 'tpl-git',
    name: 'Git Version Control MCP',
    description: 'Project git branch inspection, delta diffs, staging, and atomic commits.',
    command: 'npx',
    args: ['-y', '@modelcontextprotocol/server-git@latest'],
    envKeys: [],
    category: 'Developer Tools',
    icon: 'github',
    toolsDiscovered: ['git.status', 'git.diff', 'git.branch', 'git.commit'],
  },
  {
    id: 'tpl-fetch',
    name: 'Fetch Web Content MCP',
    description: 'Token-dense HTML-to-Markdown page conversion and header inspection.',
    command: 'npx',
    args: ['-y', '@modelcontextprotocol/server-fetch@latest'],
    envKeys: [],
    category: 'Web & Research',
    icon: 'search',
    toolsDiscovered: ['fetch.get_markdown', 'fetch.get_raw_html', 'fetch.get_headers'],
  },
  {
    id: 'tpl-brave',
    name: 'Brave Search MCP',
    description: 'Real-time live web search and recent news queries.',
    command: 'npx',
    args: ['-y', '@modelcontextprotocol/server-brave-search@latest'],
    envKeys: ['BRAVE_API_KEY'],
    category: 'Web & Research',
    icon: 'search',
    toolsDiscovered: ['brave.search_web', 'brave.search_news'],
  },
  {
    id: 'tpl-arxiv',
    name: 'ArXiv Academic Research MCP',
    description: 'Academic paper search, PDF retrieval, and abstract extraction.',
    command: 'uvx',
    args: ['mcp-server-arxiv@latest'],
    envKeys: [],
    category: 'Web & Research',
    icon: 'doc',
    toolsDiscovered: ['arxiv.search_papers', 'arxiv.get_paper_summary'],
  },
  {
    id: 'tpl-sqlite',
    name: 'SQLite Database MCP',
    description: 'SQL query execution, table inspection, and schema profiling.',
    command: 'uvx',
    args: ['mcp-server-sqlite@latest'],
    envKeys: [],
    category: 'Database & Storage',
    icon: 'db',
    toolsDiscovered: ['sqlite.read_query', 'sqlite.write_query', 'sqlite.list_tables', 'sqlite.describe_tables'],
  },
  {
    id: 'tpl-typst',
    name: 'Typst Vector Document MCP',
    description: 'Rust vector Typst compilation into PDF and DOCX reports.',
    command: 'uvx',
    args: ['typst-mcp-server@latest'],
    envKeys: [],
    category: 'Documents',
    icon: 'doc',
    toolsDiscovered: ['typst.compile_pdf', 'typst.render_template'],
  },
  {
    id: 'tpl-sequential',
    name: 'Sequential Thinking MCP',
    description: 'Multi-branch hypothesis reasoning and thought revision.',
    command: 'npx',
    args: ['-y', '@modelcontextprotocol/server-sequential-thinking@latest'],
    envKeys: [],
    category: 'Governance & Logic',
    icon: 'audit',
    toolsDiscovered: ['sequential_thinking.step'],
  },
  {
    id: 'tpl-audit',
    name: 'NAVA Audit & Invariants MCP',
    description: 'Deterministic verification of 21 Invariants, AST security scanning & grounding.',
    command: 'uvx',
    args: ['nava-audit-mcp@latest'],
    envKeys: [],
    category: 'Governance & Logic',
    icon: 'audit',
    toolsDiscovered: ['audit.verify_invariants', 'audit.security_scan', 'audit.verify_grounding'],
  },
  {
    id: 'tpl-docker',
    name: 'Docker Ephemeral Sandbox MCP',
    description: 'Ephemeral container execution with CPU/RAM resource limits and purge.',
    command: 'uvx',
    args: ['docker-sandbox-mcp@latest'],
    envKeys: [],
    category: 'Execution',
    icon: 'terminal',
    toolsDiscovered: ['docker.create_sandbox', 'docker.exec_in_sandbox', 'docker.destroy_sandbox'],
  },
  {
    id: 'tpl-playwright',
    name: 'Playwright Browser Automation MCP',
    description: 'Headless browser navigation, numbered interactive tree, and clicks.',
    command: 'npx',
    args: ['-y', '@modelcontextprotocol/server-puppeteer@latest'],
    envKeys: [],
    category: 'Browser & GUI',
    icon: 'browser',
    toolsDiscovered: ['browser.navigate', 'browser.extract_interactive_tree', 'browser.screenshot', 'browser.click'],
  },
  {
    id: 'tpl-desktop',
    name: 'Desktop OS Automation MCP',
    description: 'Display perception, high-res screenshots, coordinate clicks, and hotkeys.',
    command: 'uvx',
    args: ['desktop-automation-mcp@latest'],
    envKeys: [],
    category: 'Browser & GUI',
    icon: 'terminal',
    toolsDiscovered: ['desktop.get_screen_size', 'desktop.screenshot', 'desktop.click', 'desktop.type', 'desktop.hotkey'],
  },
  {
    id: 'tpl-gmail',
    name: 'Gmail MCP Server',
    description: 'Email search, thread reading, and governed drafting.',
    command: 'uvx',
    args: ['gmail-mcp-server@latest'],
    envKeys: ['GMAIL_CREDENTIALS_JSON'],
    category: 'Communication',
    icon: 'mail',
    toolsDiscovered: ['gmail.search', 'gmail.read'],
  },
  {
    id: 'tpl-github',
    name: 'GitHub MCP Server',
    description: 'Query pull requests, issue threads, code commits, and PR reviews.',
    command: 'npx',
    args: ['-y', '@modelcontextprotocol/server-github@latest'],
    envKeys: ['GITHUB_PERSONAL_ACCESS_TOKEN'],
    category: 'Developer Tools',
    icon: 'github',
    toolsDiscovered: ['github.search_repositories', 'github.create_pull_request', 'github.create_issue'],
  },
];
